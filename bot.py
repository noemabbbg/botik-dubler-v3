...
class add_chapters_to_manhwa(StatesGroup):
    _id = State() # это будет id определнной манхвы.
    filename = State()
    chapter_id = State()
...
async def first_step_to_add(message: types.message):
    ...
    await bot.send_message(message.from_user.id, text = 'впиши название манхвы')
...
async def add_name(message: types.message, state = add_new_manhwa.name):
    ...
    await message.reply('загрузи картинку манхвы')
...
async def get_link(message: types.Message):
    await bot.send_message(message.from_user.id, text = 'Привет! Проверка функции автоматического добавления манхвы с мангалиба :) Присылай в ответ ссылку тайтла, который хочешь добавить в формате: https://mangalib.me/kod-giass/v5/c18?ui=156163&page=1 ')
    @dp.message_handler()
    async def get_link(message: types.Message):
        urls = message.text
        print(urls)
